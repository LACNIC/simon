...
pv.Format.date = function (b) {
    ...
    c.format = c;
    c.parse = function (f) {
        var g = 1970, h = 0, i = 1, j = 0, k = 0, l = 0, q = [function () {
        }], n = pv.Format.re(b).replace(/%[a-zA-Z0-9]/g, function (p) {
            switch (p) {
                case "%b":
                    q.push(function (m) {
        ...
        (f = f.match(n)) && f.forEach(function (p, m) {
            q[m](p)
        });
        return new Date(g, h, i, j, k, l)
    };
    return c
};
        ...
        switch (b) {
            case "short":
                for (var g = /([0-9,.]+)\s*([a-z]+)/g, h, i = 0; h = g.exec(f);) {
                    var j = parseFloat(h[0].replace(",", "")), k = 0;
...
pv.Format.number = function () {
    function b(r) {
        if (Infinity > h)r = Math.round(r * i) / i;
        var s = String(Math.abs(r)).split("."), t = s[0];
        if (t.length > d)t = t.substring(t.length - d);
        if (l && t.length < c)t = (new Array(c - t.length + 1)).join(j) + t;
        if (t.length > 3)t = t.replace(/\B(?=(?:\d{3})+(?!\d))/g, n);
        if (!l && t.length < f)t = (new Array(f - t.length + 1)).join(j) + t;
        s[0] = r < 0 ? p + t + m : t;
        r = s[1] || "";
        if (r.length < g)s[1] = r + (new Array(g - r.length + 1)).join(k);
        return s.join(q)
    }

    var c = 0, d = Infinity, f = 0, g = 0, h = 0, i = 1, j = "0", k = "0", l = true, q = ".", n = ",", p = "\u2212",
        m = "";
    b.format = b;
    b.parse = function (r) {
        var s = pv.Format.re;
        r = String(r).replace(new RegExp("^(" + s(j) + ")*"), "").replace(new RegExp("(" + s(k) + ")*$"), "").split(q);
        s = r[0].replace(new RegExp(s(n), "g"), "");
        if (s.length > d)s = s.substring(s.length - d);
        r = r[1] ? Number("0." + r[1]) : 0;
        if (Infinity > h)r = Math.round(r * i) / i;
        return Math.round(s) + r
    };
    b.integerDigits = function (r, s) {
        if (arguments.length) {
            c = Number(r);
            d = arguments.length > 1 ? Number(s) : c;
            f = c + Math.floor(c / 3) * n.length;
            return this
        }
        return[c, d]
    };
    b.fractionDigits = function (r, s) {
        if (arguments.length) {
            g =
                Number(r);
            h = arguments.length > 1 ? Number(s) : g;
            i = Math.pow(10, h);
            return this
        }
        return[g, h]
    };
    b.integerPad = function (r) {
        if (arguments.length) {
            j = String(r);
            l = /\d/.test(j);
            return this
...
pv.Flatten.prototype.array = function () {
    function b(i, j) {
        if (j < f.length - 1)for (var k in i) {
            d.push(k);
            b(i[k], j + 1);
            d.pop()
    ...
    if (g) {
        function h(i, j) {
            if (g(i))c.push({keys: d.slice(), value: i}); else for (var k in i) {
                d.push(k);
                h(i[k], j + 1);
                d.pop()
            }
    ...
    return c.map(function (i) {
        for (var j = {}, k = 0; k < f.length; k++) {
            var l = f[k], q = i[k];
            j[l.name] = l.value ? l.value.call(null, q) : q
        }
        return j
    })
};
...
pv.Scale.quantitative = function () {
    ...
    function c(n) {
        var p = pv.search(d, n);
        if (p < 0)p = -p - 2;
        p = Math.max(0, Math.min(h.length - 1, p));
        return h[p]((k(n) - f[p]) / (f[p + 1] - f[p]))
    }

    var d = [0, 1], f = [0, 1], g = [0, 1], h = [pv.identity], i = Number, j = false, k = pv.identity, l = pv.identity, q = String;
    c.transform = function (n, p) {
        k = function (m) {
            return j ? -n(-m) : n(m)
        };
        l = function (m) {
    ...
    c.domain = function (n, p, m) {
        if (arguments.length) {
            var r;
            if (n instanceof Array) {
                if (arguments.length <
                    2)p = pv.identity;
                if (arguments.length < 3)m = p;
                r = n.length && p(n[0]);
                d = n.length ? [pv.min(n, p), pv.max(n, m)] : []
            } else {
                r = n;
    ...
    c.range = function () {
        if (arguments.length) {
            ...
            for (var n = 0; n < g.length -
                1; n++)h.push(pv.Scale.interpolator(g[n], g[n + 1]));
            return this
        }
        return g
    };
    c.invert = function (n) {
        ...
        if (p < 0)p = -p - 2;
        p = Math.max(0, Math.min(h.length - 1, p));
        return i(l(f[p] + (n - g[p]) / (g[p + 1] - g[p]) * (f[p + 1] - f[p])))
    };
    c.ticks = function (n) {
        var p = d[0], m = d[d.length - 1], r = m < p, s = r ? m : p;
        m = r ? p : m;
        var t = m - s;
        if (!t || !isFinite(t)) {
        ...
        if (i == b) {
            ...
            if (t > 10)switch (p) {
                ...
                case 1E3:
                    ...
                    break;
                case 1:
                    v = t > 1E3 ? 250 : t > 200 ? 100 : t > 100 ? 50 : t > 50 ? 25 : 5;
                    s.setMilliseconds(Math.floor(s.getMilliseconds() / v) * v);
                    break;
                default:
                    v = pv.logCeil(t / 15, 10);
                    if (t / v < 2)v /= 5; else if (t / v < 5)v /= 2;
        ...
        arguments.length || (n = 10);
        v = pv.logFloor(t / n, 10);
        p = n / (t / v);
        if (p <= 0.15)v *= 10; else if (p <= 0.35)v *= 5; else if (p <= 0.75)v *= 2;
        p = Math.ceil(s /
            v) * v;
        m = Math.floor(m / v) * v;
    ...
    c.tickFormat = function (n) {
        return q(n)
    };
    c.nice = function () {
        if (d.length != 2)return this;
        var n = d[0], p = d[d.length - 1], m = p < n, r = m ? p : n;
        n = m ? n : p;
        p = n - r;
        if (!p || !isFinite(p))return this;
        p = Math.pow(10, Math.round(Math.log(p) / Math.log(10)) - 1);
        d = [Math.floor(r / p) * p, Math.ceil(n / p) * p];
        m && d.reverse();
        f = d.map(k);
        return this
    };
    c.by = function (n) {
        function p() {
            return c(n.apply(this,
                arguments))
        }

        for (var m in c)p[m] = c[m];
        return p
    };
...
pv.Scale.log = function () {
    ...
    b.ticks = function () {
        var h = b.domain(), i = h[0] < 0, j = Math.floor(i ? -f(-h[0]) : f(h[0])), k = Math.ceil(i ? -f(-h[1]) : f(h[1])), l = [];
        if (i)for (l.push(-g(-j)); j++ < k;)for (i = c - 1; i > 0; i--)l.push(-g(-j) * i); else {
            for (; j < k; j++)for (i = 1; i < c; i++)l.push(g(j) * i);
            l.push(g(j))
        }
        for (j = 0; l[j] < h[0]; j++);
        for (k = l.length; l[k - 1] > h[1]; k--);
        return l.slice(j, k)
    };
...
pv.color = function (b) {
    ...
    if (c) {
        ...
        switch (c[1]) {
            ...
            function h(k) {
                var l = parseFloat(k);
                return k[k.length - 1] == "%" ? Math.round(l * 2.55) : l
            }
...
pv.SvgScene.pathBasis = function () {
    ...
    var c = [
        ...
        var j = b(c[1], f, g, h, i), k = b(c[2], f, g, h, i);
        f = b(c[3], f, g, h, i);
        return"C" + j.x + "," + j.y + "," + k.x + "," + k.y + "," + f.x + "," + f.y
    };
    d.segment = function (f, g, h, i) {
        var j = b(c[0], f, g, h, i), k = b(c[1], f, g, h, i), l = b(c[2], f, g, h, i);
        f = b(c[3], f, g, h, i);
        return"M" + j.x + "," + j.y + "C" + k.x + "," + k.y +
            "," + l.x + "," + l.y + "," + f.x + "," + f.y
    };
    return d
}();
...
pv.SvgScene.curveHermite = function (b, c) {
    ...
    var d = b.length != c.length, f = "", g = b[0], h = b[1], i = c[0], j = i, k = 1;
    if (d) {
...
pv.SvgScene.curveHermiteSegments = function (b, c) {
    if (c.length < 1 || b.length != c.length && b.length != c.length + 2)return[];
    var d = b.length != c.length, f = [], g = b[0], h = g, i = c[0], j = i, k = 1;
    if (d) {
        h = b[1];
        f.push("M" + g.left + "," + g.top + "Q" + (h.left - j.x * 2 / 3) + "," + (h.top - j.y * 2 / 3) + "," + h.left + "," + h.top);
        k = 2
    }
    for (var l = 1; l < c.length; l++, k++) {
        g = h;
        i = j;
        h = b[k];
        j = c[l];
...
pv.SvgScene.monotoneTangents = function (b) {
    ...
    for (h = 0; h < b.length - 1; h++)if (!(Math.abs(f[h]) < 1.0E-5 || Math.abs(f[h + 1]) < 1.0E-5)) {
        var i = f[h] / d[h], j = f[h + 1] / d[h], k = i * i + j * j;
        if (k > 9) {
            k = 3 / Math.sqrt(k);
            f[h] =
                k * i * d[h];
...
pv.SvgScene.area = function (b) {
    ...
    function d(n, p) {
        for (var m = [], r = [], s = p; n <= s; n++, p--) {
            var t = b[p];
            m.push(b[n]);
            r.push({left: t.left + t.width, top: t.top + t.height})
        }
        if (g.interpolate ==
            "basis") {
            n = pv.SvgScene.curveBasis(m);
            p = pv.SvgScene.curveBasis(r)
        } else if (g.interpolate == "cardinal") {
            n = pv.SvgScene.curveCardinal(m, g.tension);
            p = pv.SvgScene.curveCardinal(r, g.tension)
        } else {
            n = pv.SvgScene.curveMonotone(m);
            p = pv.SvgScene.curveMonotone(r)
        }
        return m[0].left + "," + m[0].top + n + "L" + r[0].left + "," + r[0].top + p
    }
    ...
    for (var j =
        ...
        k = b[l];
        if (k.width || k.height) {
            for (var q = l + 1; q < b.length; q++) {
                k = b[q];
                if (!k.width && !k.height)break
            }
            l && g.interpolate != "step-after" && l--;
            q < b.length && g.interpolate != "step-before" && q++;
            j.push((q - l > 2 && (g.interpolate == "basis" || g.interpolate == "cardinal" || g.interpolate == "monotone") ? d : c)(l, q - 1));
            l = q - 1
        }
    }
...
pv.SvgScene.areaSegment = function (b) {
    ...
    for (i = b.length - 1; h < i; h++) {
        d = b[h];
        var k = b[h + 1];
        if (d.visible && k.visible) {
            var l = d.fillStyle, q = d.strokeStyle;
            if (l.opacity || q.opacity) {
                if (f) {
                    j = f[h];
                    k = "L" + g[i - h - 1].substr(1);
                    j = j + k + "Z"
                } else {
                    var n = d;
                    j = k;
                    ...
                    j = "M" + d.left + "," + n.top + "L" + k.left + "," + j.top + "L" + (k.left + k.width) + "," + (j.top + j.height) + "L" + (d.left + d.width) + "," + (n.top + n.height) + "Z"
                }
                c = this.expect(c, "path", {"shape-rendering": d.antialias ? null :
                    "crispEdges", "pointer-events": d.events, cursor: d.cursor, d: j, fill: l.color, "fill-opacity": l.opacity || null, stroke: q.color, "stroke-opacity": q.opacity || null, "stroke-width": q.opacity ? d.lineWidth / this.scale : null});
                c = this.append(c, b, h)
            }
        }
    }
        ...
        if (f.visible) {
            ...
            if (g.opacity && f.text) {
                ...
                c = this.expect(c, "text", {"pointer-events": f.events, cursor: f.cursor, x: h,
                    y: i, dy: j, transform: "translate(" + f.left + "," + f.top + ")" + (f.textAngle ? " rotate(" + 180 * f.textAngle / Math.PI + ")" : "") + (this.scale != 1 ? " scale(" + 1 / this.scale + ")" : ""), fill: g.color, "fill-opacity": g.opacity || null, "text-anchor": k}, {font: f.font, "text-shadow": f.textShadow, "text-decoration": f.textDecoration});
...
pv.SvgScene.lineSegment = function (b) {
    ...
    for (var g = b.length - 1; d < g; d++) {
        ...
        if (h.visible && i.visible) {
            var j = h.strokeStyle, k = pv.Color.transparent;
            if (j.opacity) {
                ...
                c = this.expect(c, "path", {"shape-rendering": h.antialias ? null : "crispEdges", "pointer-events": h.events, cursor: h.cursor, d: i, fill: k.color, "fill-opacity": k.opacity || null, stroke: j.color, "stroke-opacity": j.opacity || null, "stroke-width": j.opacity ? h.lineWidth / this.scale : null, "stroke-linejoin": h.lineJoin});
                c = this.append(c, b, d)
            }
        }
    }
...
pv.SvgScene.panel = function (b) {
    for (var c = b.$g, d = c && c.firstChild, f = 0; f < b.length; f++) {
        ...
        if (g.visible) {
            ...
            d = this.fill(d, b, f);
            var j = this.scale, k = g.transform, l = g.left + k.x, q = g.top + k.y;
            this.scale *= k.k;
            for (h = 0; h < g.children.length; h++) {
                g.children[h].$g = d = this.expect(d, "g", {transform: "translate(" + l + "," + q + ")" + (k.k != 1 ? " scale(" + k.k + ")" : "")});
...
pv.SvgScene.wedge = function (b) {
    for (var c = b.$g.firstChild, d = 0; d < b.length; d++) {
        ...
        if (f.visible) {
            var g = f.fillStyle, h = f.strokeStyle;
            if (g.opacity || h.opacity) {
                var i = f.innerRadius, j = f.outerRadius, k = Math.abs(f.angle);
                if (k >= 2 * Math.PI)i = i ? "M0," + j + "A" + j + "," + j + " 0 1,1 0," + -j + "A" + j + "," + j + " 0 1,1 0," + j + "M0," + i + "A" + i + "," + i + " 0 1,1 0," + -i + "A" + i + "," + i + " 0 1,1 0," + i + "Z" : "M0," + j + "A" + j + "," + j + " 0 1,1 0," + -j + "A" + j + "," + j + " 0 1,1 0," + j + "Z"; else {
                    var l = Math.min(f.startAngle, f.endAngle), q = Math.max(f.startAngle, f.endAngle),
                        n = Math.cos(l), p = Math.cos(q);
                    l = Math.sin(l);
                    q = Math.sin(q);
                    i = i ? "M" + j * n + "," + j * l + "A" + j + "," + j + " 0 " + (k < Math.PI ? "0" : "1") + ",1 " + j * p + "," + j * q + "L" + i * p + "," + i * q + "A" + i + "," + i + " 0 " + (k < Math.PI ? "0" : "1") + ",0 " + i * n + "," + i * l + "Z" : "M" + j * n + "," + j * l + "A" + j + "," + j + " 0 " + (k < Math.PI ? "0" : "1") + ",1 " + j * p + "," + j * q + "L0,0Z"
                }
...
a.anchor = function (b) {
    b || (b = "center");
    return(new pv.Anchor(this)).name(b).data(function () {
        return this.scene.target.map(function (c) {
            return c.data
        })
    }).visible(function () {
        return this.scene.target[this.index].visible
    }).left(function () {
        var c = this.scene.target[this.index], d = c.width || 0;
        switch (this.name()) {
            case "bottom":
            case "top":
            case "center":
        ...
        var c = this.scene.target[this.index], d = c.height || 0;
        switch (this.name()) {
            case "left":
            case "right":
            case "center":
        ...
        var c = this.scene.target[this.index];
        return this.name() == "left" ? c.right + (c.width || 0) : null
    }).bottom(function () {
        var c = this.scene.target[this.index];
        return this.name() == "top" ? c.bottom + (c.height || 0) : null
    }).textAlign(function () {
        switch (this.name()) {
            case "bottom":
            case "top":
...
a.instances = function (b) {
    for (var c = this, d = [], f; !(f = c.scene);) {
        b = b.parent;
        d.push({index: b.index, childIndex: c.childIndex});
        c = c.parent
    }
    for (; d.length;) {
        b = d.pop();
        f = f[b.index].children[b.childIndex]
    }
    if (this.hasOwnProperty("index")) {
        d = pv.extend(f[this.index]);
        d.right = d.top = d.left = d.bottom = 0;
        return[d]
    }
    return f
};
...
a.render = function () {
    function b(i, j, k) {
        ...
        if (j < g.length) {
            f.unshift(null);
            if (i.hasOwnProperty("index"))c(i, j, k); else {
                for (var l = 0, q = i.scene.length; l < q; l++) {
                    i.index = l;
                    c(i, j, k)
                }
                delete i.index
    ...
    function c(i, j, k) {
        var l = i.scene[i.index], q;
        if (l.visible) {
            var n = g[j], p = i.children[n];
            for (q = 0; q < n; q++)i.children[q].scene = l.children[q];
            f[0] = l.data;
            if (p.scene)b(p, j + 1, k * l.transform.k); else {
                p.scene = l.children[n];
                b(p, j + 1, k * l.transform.k);
                delete p.scene
            }
            for (q = 0; q < n; q++)delete i.children[q].scene
        }
    }
...
a.bind = function () {
    function b(j) {
        do for (var k = j.$properties, l = k.length - 1; l >= 0; l--) {
            var q = k[l];
            if (!(q.name in c)) {
                c[q.name] = q;
                switch (q.name) {
...
a.build = function () {
    ...
    if (this.target)b.target = this.target.instances(b);
    ...
    d = this.binds.data;
    d = d.type &
        1 ? d.value.apply(this, c) : d.value;
    c.unshift(null);
    b.length = d.length;
...
a.buildImplied = function (b) {
    var c = b.left, d = b.right, f = b.top, g = b.bottom, h = this.properties, i = h.width ? b.width : 0, j = h.height ? b.height : 0, k = this.parent ? this.parent.width() : i + c + d;
    if (i == null)i = k - (d = d || 0) - (c = c || 0); else if (d == null)if (c == null)c = d = (k - i) / 2; else d = k - i - (c = c || 0); else if (c == null)c = k - i - d;
    k = this.parent ? this.parent.height() : j + f + g;
    if (j == null)j = k - (f = f || 0) - (g = g || 0); else if (g == null)g = f == null ? (f = (k - j) / 2) : k - j - (f = f || 0); else if (f == null)f = k - j - g;
    b.left = c;
    b.right = d;
    b.top = f;
    b.bottom = g;
    ...
    if (h.height)b.height =
        j;
    if (h.textStyle && !b.textStyle)b.textStyle = pv.Color.transparent;
...
a.context = function (b, c, d) {
    function f(l, q) {
        ...
        if (l) {
            ...
            for (q = 1; l > 0; l--) {
                p = m[l];
                p.scale = q;
                q *= p.scene[p.index].transform.k
            }
            if (n.children) {
                l = 0;
                for (m = n.children.length; l < m; l++) {
                    p = n.children[l];
                    p.scene = n.scene[n.index].children[l];
                    p.scale = q
                }
            }
        }
    ...
    function g(l) {
        if (l) {
            l = l.mark;
            var q;
            if (l.children)for (var n = 0, p = l.children.length; n < p; n++) {
                q = l.children[n];
                delete q.scene;
                delete q.scale
            }
    ...
    var h = pv.Mark.prototype, i = pv.Mark.stack, j = pv.Mark.scene, k = h.index;
    g(j, k);
    f(b, c);
    try {
        d.apply(this, i)
...
pv.Anchor.prototype.extend = function (b) {
    this.proto = b;
    return this
};
pv.Area = function () {
...
pv.Area.prototype.anchor = function (b) {
    return pv.Mark.prototype.anchor.call(this, b).interpolate(function () {
...
pv.Dot.prototype.anchor = function (b) {
    return pv.Mark.prototype.anchor.call(this, b).left(function () {
        var c = this.scene.target[this.index];
        switch (this.name()) {
            case "bottom":
            case "top":
        ...
        return c.left + c.radius
    }).right(function () {
        var c = this.scene.target[this.index];
        return this.name() == "left" ? c.right + c.radius : null
    }).top(function () {
        var c = this.scene.target[this.index];
        switch (this.name()) {
            case "left":
        ...
        var c = this.scene.target[this.index];
        return this.name() == "top" ? c.bottom + c.radius : null
    }).textAlign(function () {
        switch (this.name()) {
            case "left":
                return"right";
...
a.anchor = function (b) {
    function c(g) {
        return g.innerRadius || g.angle < 2 * Math.PI
    }

    function d(g) {
        return(g.innerRadius + g.outerRadius) / 2
    }
    ...
    return pv.Mark.prototype.anchor.call(this, b).left(function () {
        var g = this.scene.target[this.index];
        if (c(g))switch (this.name()) {
            case "outer":
                return g.left + g.outerRadius * Math.cos(f(g));
            case "inner":
                return g.left + g.innerRadius * Math.cos(f(g));
            case "start":
                return g.left + d(g) * Math.cos(g.startAngle);
            case "center":
                return g.left + d(g) * Math.cos(f(g));
            case "end":
                return g.left + d(g) * Math.cos(g.endAngle)
        }
        return g.left
    }).top(function () {
        var g = this.scene.target[this.index];
        if (c(g))switch (this.name()) {
            case "outer":
                return g.top + g.outerRadius * Math.sin(f(g));
            case "inner":
                return g.top + g.innerRadius * Math.sin(f(g));
            case "start":
                return g.top + d(g) * Math.sin(g.startAngle);
            case "center":
                return g.top + d(g) * Math.sin(f(g));
            case "end":
                return g.top + d(g) * Math.sin(g.endAngle)
        }
        return g.top
    }).textAlign(function () {
        var g = this.scene.target[this.index];
        if (c(g))switch (this.name()) {
            case "outer":
                return pv.Wedge.upright(f(g)) ?
                    "right" : "left";
            case "inner":
                return pv.Wedge.upright(f(g)) ? "left" : "right"
        }
        return"center"
    }).textBaseline(function () {
        var g = this.scene.target[this.index];
        if (c(g))switch (this.name()) {
            case "start":
                return pv.Wedge.upright(g.startAngle) ? "top" : "bottom";
            case "end":
                return pv.Wedge.upright(g.endAngle) ? "bottom" : "top"
        }
        return"middle"
    }).textAngle(function () {
        var g = this.scene.target[this.index], h = 0;
        if (c(g))switch (this.name()) {
            case "center":
            case "inner":
            case "outer":
        ...
        return pv.Wedge.upright(h) ? h : h + Math.PI
    })
};
...
pv.Quadtree = function (b) {
    function c(l, q, n, p, m, r) {
        if (!(isNaN(q.x) || isNaN(q.y)))if (l.leaf)if (l.p) {
            if (!(Math.abs(l.p.x - q.x) + Math.abs(l.p.y - q.y) < 0.01)) {
                var s = l.p;
                l.p = null;
                d(l, s, n, p, m, r)
            }
            d(l, q, n, p, m, r)
        } else l.p = q; else d(l, q, n, p, m, r)
    }

    function d(l, q, n, p, m, r) {
        var s = (n + m) * 0.5, t = (p + r) * 0.5, x = q.x >= s, u = q.y >= t;
        l.leaf = false;
        switch ((u << 1) + x) {
            case 0:
                l = l.c1 || (l.c1 = new pv.Quadtree.Node);
                break;
            case 1:
                l = l.c2 || (l.c2 = new pv.Quadtree.Node);
                break;
            case 2:
                l = l.c3 || (l.c3 = new pv.Quadtree.Node);
                break;
            case 3:
                l = l.c4 || (l.c4 = new pv.Quadtree.Node);
                break
        }
        if (x)n = s; else m = s;
        if (u)p = t; else r = t;
...
pv.Force.charge = function (b) {
    ...
    function d(l, q, n, p, m, r) {
        var s = l.cx - q.x, t = l.cy - q.y, x = 1 / Math.sqrt(s * s + t * t);
        if (l.leaf && l.p != q || (m - n) * x < j) {
            if (!(x < i)) {
                if (x > g)x = g;
                l = l.cn * x * x * x;
                s = s * l;
            ...
            var u = (n + m) * 0.5, o = (p + r) * 0.5;
            l.c1 && d(l.c1, q, n, p, u, o);
            l.c2 && d(l.c2, q, u, p,
                m, o);
            l.c3 && d(l.c3, q, n, o, u, r);
            l.c4 && d(l.c4, q, u, o, m, r);
            if (!(x < i)) {
                if (x > g)x = g;
                if (l.p && l.p != q) {
                    l = b * x * x * x;
    ...
    var f = 2, g = 1 / f, h = 500, i = 1 / h, j = 0.9, k = {};
    arguments.length || (b = -40);
    k.constant = function (l) {
        if (arguments.length) {
            b = Number(l);
            return k
        }
    ...
    k.apply = function (l, q) {
        c(q.root);
        for (l = l; l; l = l.next)d(q.root,
            l, q.xMin, q.yMin, q.xMax, q.yMax)
    };
    return k
};
...
pv.Force.spring = function (b) {
    ...
    h.apply = function () {
        for (var i = 0; i < f.length; i++) {
            var j =
                f[i].sourceNode, k = f[i].targetNode, l = j.x - k.x, q = j.y - k.y, n = Math.sqrt(l * l + q * q), p = n ? 1 / n : 1;
            p = (b * g[i] * (n - d) + c * g[i] * (l * (j.vx - k.vx) + q * (j.vy - k.vy)) * p) * p;
            l = -p * (n ? l : 0.01 * (0.5 - Math.random()));
            q = -p * (n ? q : 0.01 * (0.5 - Math.random()));
            j.fx += l;
            j.fy += q;
            k.fx -= l;
            k.fy -= q
...
pv.Constraint.collision = function (b) {
    function c(l, q, n, p, m, r) {
        if (!l.leaf) {
            var s = (n + m) * 0.5, t = (p + r) * 0.5, x = t < j, u = s > g, o = s < i;
            if (t > h) {
                l.c1 && u && c(l.c1, q, n, p, s, t);
                l.c2 && o && c(l.c2, q, s, p, m, t)
            }
            if (x) {
                l.c3 && u && c(l.c3, q, n, t, s, r);
                l.c4 && o && c(l.c4, q, s, t, m, r)
            }
        }
        if (l.p && l.p != q) {
            n = q.x - l.p.x;
    ...
    k.apply =
        function (l, q) {
            ...
            for (var r = 0; r < d; r++)for (n = l; n; n = n.next) {
                p = (f = b(n)) + m;
                g = n.x - p;
                i = n.x + p;
                h = n.y - p;
                j = n.y + p;
                c(q.root, n, q.xMin, q.yMin, q.xMax, q.yMax)
            }
        };
    return k
};
...
pv.Layout.Hierarchy.NodeLink = {buildImplied: function (b) {
    function c(m) {
        return m.parentNode ? m.depth * (n - q) + q : 0
    }
    ...
    function f(m) {
        switch (i) {
            ...
            case "radial":
                return k / 2 + c(m) * Math.cos(m.midAngle)
        }
    }
        ...
        switch (i) {
            ...
            case "bottom":
                return l -
                    m.depth * l;
            case "radial":
                return l / 2 + c(m) * Math.sin(m.midAngle)
        }
    }

    var h = b.nodes, i = b.orient, j = /^(top|bottom)$/.test(i), k = b.width, l = b.height;
    if (i == "radial") {
        var q = b.innerRadius, n = b.outerRadius;
        if (q == null)q = 0;
        if (n == null)n = Math.min(k, l) / 2
    }
    for (b = 0; b < h.length; b++) {
        var p = h[b];
        p.midAngle = i == "radial" ? d(p) : j ? Math.PI / 2 : 0;
        p.x = f(p);
        p.y = g(p);
...
pv.Layout.Hierarchy.Fill = {constructor: function () {
    ...
    function d(o) {
        switch (n) {
            case "left":
                return c(o.minDepth, s) * m;
            case "right":
                return(1 - c(o.maxDepth, s)) * m;
            case "top":
                return o.minBreadth * m;
            case "bottom":
                return(1 - o.maxBreadth) * m;
            case "radial":
                return m / 2
        }
    }
    ...
    function f(o) {
        switch (n) {
            case "left":
                return o.minBreadth * r;
            case "right":
                return(1 - o.maxBreadth) * r;
            case "top":
                return c(o.minDepth, s) * r;
            case "bottom":
                return(1 - c(o.maxDepth, s)) * r;
            case "radial":
                return r / 2
        }
    }
    ...
    function g(o) {
        switch (n) {
            case "left":
            case "right":
                return(o.maxDepth - o.minDepth) / (1 + s) * m;
            case "top":
            case "bottom":
                return(o.maxBreadth -
                    o.minBreadth) * m;
            case "radial":
                return o.parentNode ? (o.innerRadius + o.outerRadius) * Math.cos(o.midAngle) : 0
        }
    }
    ...
    function h(o) {
        switch (n) {
            case "left":
            case "right":
                return(o.maxBreadth - o.minBreadth) * r;
            case "top":
            case "bottom":
                return(o.maxDepth - o.minDepth) / (1 + s) * r;
            case "radial":
                return o.parentNode ? (o.innerRadius + o.outerRadius) * Math.sin(o.midAngle) : 0
        }
    }

    function i(o) {
        return Math.max(0, c(o.minDepth, s / 2)) * (x - t) + t
    }

    function j(o) {
        return c(o.maxDepth, s / 2) * (x - t) + t
    }

    function k(o) {
        return(o.parentNode ? o.minBreadth - 0.25 : 0) *
            2 * Math.PI
    }

    function l(o) {
        return(o.parentNode ? o.maxBreadth - o.minBreadth : 1) * 2 * Math.PI
    }

    var q = b.nodes, n = b.orient, p = /^(top|bottom)$/.test(n), m = b.width, r = b.height, s = -q[0].minDepth;
    if (n == "radial") {
        var t = b.innerRadius, x = b.outerRadius;
        if (t == null)t = 0;
        if (t)s *= 2;
    ...
    for (b = 0; b < q.length; b++) {
        ...
        if (n == "radial") {
            ...
            u.startAngle = k(u);
            u.angle = l(u);
            u.midAngle = u.startAngle + u.angle / 2
        } else u.midAngle = p ? -Math.PI / 2 : 0;
        u.dx = g(u);
        u.dy = h(u)
    }
}};
...
pv.Layout.Stack = function () {
    ...
    this.buildImplied = function (i) {
        h.call(this, i);
        var j = i.layers, k = j.length, l, q = i.orient, n = /^(top|bottom)\b/.test(q), p = this.parent[n ? "height" : "width"](), m = [], r = [], s = [], t = pv.Mark.stack, x = {parent: {parent: this}};
        t.unshift(null);
        g = [];
        for (var u = 0; u < k; u++) {
            s[u] = [];
            ...
            t[0] = j[u];
            g[u] =
                this.$values.apply(x.parent, t);
            if (!u)l = g[u].length;
            t.unshift(null);
            for (var o = 0; o < l; o++) {
                t[0] = g[u][o];
                x.index = o;
                u || (m[o] = this.$x.apply(x, t));
                s[u][o] = this.$y.apply(x, t)
            }
            t.shift()
        }
        t.shift();
        switch (i.order) {
            case "inside-out":
                ...
                x = pv.range(k).sort(function (A, D) {
                    return v[A] - v[D]
                });
                j = s.map(function (A) {
                    return pv.sum(A)
        ...
        switch (i.offset) {
            case "silohouette":
                for (o = 0; o < l; o++) {
                    for (u = x = 0; u < k; u++)x += s[u][o];
                    r[j[0]][o] = (p - x) / 2
                }
                break;
            case "wiggle":
                for (u = x = 0; u < k; u++)x += s[u][0];
                r[j[0]][0] = x = (p - x) / 2;
                for (o = 1; o < l; o++) {
                    t = p = 0;
                    w = m[o] - m[o - 1];
                    for (u = 0; u < k; u++)p += s[u][o];
                    for (u = 0; u < k; u++) {
                        y = (s[j[u]][o] - s[j[u]][o - 1]) / (2 * w);
                        for (i = 0; i < u; i++)y += (s[j[i]][o] - s[j[i]][o - 1]) / w;
                        t += y * s[j[u]][o]
                    }
                    r[j[0]][o] = x -= p ? t / p * w : 0
                }
            ...
            case "expand":
                for (o = 0; o < l; o++) {
                    for (u = i = r[j[0]][o] = 0; u < k; u++)i += s[u][o];
                    if (i) {
                        i = p / i;
                        for (u = 0; u < k; u++)s[u][o] *= i
                    } else {
                        i = p / k;
                        for (u = 0; u < k; u++)s[u][o] = i
                    }
...
a.buildImplied = function (b) {
    function c(r, s, t, x, u, o, v) {
        ...
        if (z)if (t)z.dx += o - y; else z.dy += v - y
    }

    function d(r, s) {
        for (var t = -Infinity, x = Infinity, u = 0, o = 0; o < r.length; o++) {
            var v = r[o].size;
            if (v < x)x = v;
            if (v > t)t = v;
            u += v
        ...
        u *= u;
        s *= s;
        return Math.max(s * t / u, u / (s * x))
    }

    function f(r, s) {
        function t(A) {
            var D = o == y, G = pv.sum(A, n), E = y ? p(G / y) : 0;
            c(A, G, D, x, u, D ? o : E, D ? E : v);
            if (D) {
                u += E;
                v -= E
            } else {
        ...
        var x = r.x + j, u = r.y + l, o = r.dx - j - k, v = r.dy - l - q;
        if (m != "squarify")c(r.childNodes, r.size, m == "slice" ? true : m == "dice" ? false : s & 1, x, u, o, v); else {
            var w = [];
            s = Infinity;
            var y = Math.min(o, v), z = o * v / r.size;
            if (!(r.size <= 0)) {
                ...
                if (t(w))for (s = 0; s < w.length; s++)w[s].dy += v; else for (s = 0; s < w.length; s++)w[s].dx +=
                    o
            }
        }
    }
    ...
    if (!pv.Layout.Hierarchy.prototype.buildImplied.call(this, b)) {
        var g = this, h = b.nodes[0], i = pv.Mark.stack, j = b.paddingLeft, k = b.paddingRight, l = b.paddingTop, q = b.paddingBottom, n = function (r) {
            return r.size
        }, p = b.round ? Math.round : Number, m = b.mode;
        i.unshift(null);
        h.visitAfter(function (r, s) {
            r.depth = s;
            r.x = r.y = r.dx = r.dy = 0;
            r.size = r.firstChild ? pv.sum(r.childNodes, function (t) {
                return t.size
...
pv.Layout.Tree.prototype.buildImplied = function (b) {
    function c(o) {
        ...
        if (o.firstChild) {
            ...
            if (v = o.previousSibling) {
                o.prelim = v.prelim + l(o.depth, true);
                o.mod = o.prelim - w
            } else o.prelim = w
        } else if (v = o.previousSibling)o.prelim = v.prelim + l(o.depth, true)
    }

    function d(o, v, w) {
        o.breadth = o.prelim + v;
        v += o.mod;
        for (o = o.firstChild; o; o = o.nextSibling)d(o, v, w)
    }
    ...
    function f(o, v) {
        ...
        if (w) {
            ...
            for (y = g(y); C && y;) {
                ...
                z = h(z);
                z.ancestor = o;
                var B = C.prelim + G - (y.prelim + A) + l(C.depth, false);
                if (B > 0) {
                    i(k(C, o, v), o, B);
                    A += B;
                    D += B
    ...
    function g(o) {
        return o.firstChild || o.thread
    }

    function h(o) {
        return o.lastChild || o.thread
    }

    function i(o, v, w) {
        var y = v.number - o.number;
        v.change -= w / y;
        v.shift += w;
        o.change +=
            w / y;
    ...
    function j(o) {
        var v = 0, w = 0;
        for (o = o.lastChild; o; o = o.previousSibling) {
            o.prelim += v;
            o.mod += v;
            w += o.change;
            v += o.shift + w
    ...
    function k(o, v, w) {
        return o.ancestor.parentNode == v.parentNode ? o.ancestor : w
    }

    function l(o, v) {
        return(v ? 1 : t + 1) / (m == "radial" ? o : 1)
    }

    function q(o) {
        return m == "radial" ? o.breadth / r : 0
    }
    ...
    function n(o) {
        switch (m) {
            ...
            case "top":
            case "bottom":
                return o.breadth + x / 2;
            case "radial":
                return x / 2 + o.depth * Math.cos(q(o))
        }
    }
    ...
    function p(o) {
        switch (m) {
            ...
            case "bottom":
                return u - o.depth;
            case "radial":
                return u / 2 + o.depth * Math.sin(q(o))
        }
    }

    if (!pv.Layout.Hierarchy.prototype.buildImplied.call(this, b)) {
        var m = b.orient, r = b.depth, s = b.breadth, t = b.group, x = b.width, u = b.height;
        b = b.nodes[0];
        b.visitAfter(function (o, v) {
            o.ancestor = o;
            o.prelim = 0;
            o.mod = 0;
            o.change = 0;
            o.shift = 0;
            o.number = o.previousSibling ? o.previousSibling.number + 1 : 0;
            o.depth = v
        });
        c(b);
        d(b, -b.prelim, 0);
        b.visitAfter(function (o) {
            o.breadth *= s;
            o.depth *= r;
            o.midAngle = q(o);
            o.x = n(o);
            o.y = p(o);
            if (o.firstChild)o.midAngle += Math.PI;
            delete o.breadth;
            delete o.depth;
            delete o.ancestor;
            delete o.prelim;
...
pv.Layout.Indent.prototype.buildImplied = function (b) {
    function c(i, j, k) {
        ...
        i.y = h + j++ * d;
        i.midAngle = 0;
        for (i = i.firstChild; i; i = i.nextSibling)j = c(i, j, k);
        return j
    }
...
pv.Layout.Pack.prototype.buildImplied = function (b) {
    function c(n) {
        ...
        for (var m = 0, r = n.length; m < r; m++) {
            var s = n[m];
            if (!s.firstChild)s.radius = i.$radius.apply(i, (p[0] = s, p))
        }
        p.shift()
    }
    ...
    function f(n) {
        function p(B) {
            t = Math.min(B.x - B.radius, t);
            x = Math.max(B.x + B.radius, x);
            u = Math.min(B.y - B.radius, u);
            o = Math.max(B.y + B.radius, o)
        }
        ...
        var t = Infinity, x = -Infinity, u = Infinity, o = -Infinity, v, w, y, z, C;
        v = n[0];
        v.x = -v.radius;
        v.y = 0;
        p(v);
    ...
    function g(n, p, m) {
        var r = p.radius + m.radius, s = n.radius + m.radius, t = p.x - n.x;
        p = p.y - n.y;
        var x = Math.sqrt(t *
            t + p * p), u = (s * s + x * x - r * r) / (2 * s * x);
        r = Math.acos(u);
        u = u * s;
        s = Math.sin(r) * s;
        t /= x;
    ...
    function h(n, p, m, r) {
        for (var s = n.firstChild; s; s = s.nextSibling) {
            s.x += n.x;
            s.y += n.y;
            h(s, p, m, r)
        }
    ...
    if (!pv.Layout.Hierarchy.prototype.buildImplied.call(this, b)) {
        var i = this, j = b.nodes, k = j[0];
        c(j);
        k.x = 0;
        k.y = 0;
        k.radius = d(k);
        j = this.width();
        var l = this.height(), q = 1 / Math.max(2 * k.radius / j, 2 * k.radius / l);
        h(k, j / 2, l / 2, q)
    }
};
...
pv.Layout.Force.prototype.buildImplied = function (b) {
    ...
    if (pv.Layout.Network.prototype.buildImplied.call(this, b)) {
        ...
        for (var d = this, f = b.nodes, g = b.links, h = b.iterations, i = b.width, j = b.height, k = 0, l; k < f.length; k++) {
            l = f[k];
            if (isNaN(l.x))l.x = i / 2 + 40 * Math.random() - 20;
            if (isNaN(l.y))l.y = j / 2 + 40 * Math.random() - 20
        }
        l = pv.simulation(f);
        l.force(pv.Force.drag(b.dragConstant));
        l.force(pv.Force.charge(b.chargeConstant).domain(b.chargeMinDistance,
            b.chargeMaxDistance).theta(b.chargeTheta));
        l.force(pv.Force.spring(b.springConstant).damping(b.springDamping).length(b.springLength).links(g));
        l.constraint(pv.Constraint.position());
        b.bound && l.constraint(pv.Constraint.bound().x(6, i - 6).y(6, j - 6));
        if (h == null) {
            l.step();
            l.step();
            b.$force = this.binds.$force = {next: this.binds.$force, nodes: f, min: 1.0E-4 * (g.length + 1), sim: l};
            if (!this.$timer)this.$timer = setInterval(function () {
                for (var q = false, n = d.binds.$force; n; n = n.next)if (pv.max(n.nodes, c) > n.min) {
                    n.sim.step();
                    q = true
                }
                q && d.render()
            }, 42)
        } else for (k = 0; k < h; k++)l.step()
    }
};
...
pv.Layout.Cluster.prototype.buildImplied = function (b) {
    if (!pv.Layout.Hierarchy.prototype.buildImplied.call(this, b)) {
        ...
        c.visitAfter(function (k) {
            if (k.firstChild)k.depth = 1 + pv.max(k.childNodes, function (l) {
                return l.depth
            }); else {
                if (d && j != k.parentNode) {
                    j = k.parentNode;
        ...
        j = undefined;
        c.visitAfter(function (k) {
            if (k.firstChild)k.breadth = pv.mean(k.childNodes, function (l) {
                return l.breadth
            }); else {
                if (d && j != k.parentNode) {
                    j = k.parentNode;
        ...
        c.visitAfter(function (k) {
            k.minBreadth = k.firstChild ? k.firstChild.minBreadth : k.breadth - f / 2;
            k.maxBreadth = k.firstChild ? k.lastChild.maxBreadth : k.breadth + f / 2
        });
        c.visitBefore(function (k) {
            k.minDepth = k.parentNode ? k.parentNode.maxDepth : 0;
            k.maxDepth = k.parentNode ? k.depth + c.depth : k.minDepth + 2 * c.depth
        });
        c.minDepth = -g;
...
pv.Layout.Partition.prototype.buildImplied = function (b) {
    if (!pv.Layout.Hierarchy.prototype.buildImplied.call(this, b)) {
        ...
        d.visitAfter(function (i, j) {
            if (j > g)g = j;
            i.size = i.firstChild ? pv.sum(i.childNodes, function (k) {
                return k.size
        ...
        d.visitBefore(function (i) {
            for (var j = i.minBreadth, k = i.maxBreadth - j, l = i.firstChild; l; l = l.nextSibling) {
                l.minBreadth = j;
                l.maxBreadth = j += l.size / i.size * k;
                l.breadth = (j + l.minBreadth) / 2
            }
        });
        d.visitAfter(function (i, j) {
            i.minDepth = (j - 1) * h;
    ...
    function f(m) {
        switch (h) {
            ...
            case "right":
                return m * l;
            case "radial":
                return l / 2 + q * Math.sin(c(m))
        }
    }

    if (!pv.Layout.Network.prototype.buildImplied.call(this, b)) {
        var g = b.nodes, h = b.orient, i = this.$sort, j = pv.range(g.length), k = b.width, l = b.height, q = Math.min(k, l) / 2;
        i && j.sort(function (m, r) {
            return i(g[m], g[r])
        });
        for (b = 0; b < g.length; b++) {
            var n = g[j[b]], p = n.breadth = (b + 0.5) / g.length;
            n.x = d(p);
            n.y = f(p);
            n.midAngle = c(p)
        }
...
pv.Layout.Horizon = function () {
    ...
    this.buildImplied = function (k) {
        j.call(this, k);
        c = k.bands;
        d = k.mode;
        f = Math.round((d == "color" ? 0.5 : 1) * k.height);
        g = k.backgroundStyle;
        h = pv.ramp(g, k.negativeStyle).domain(0, c);
        i = pv.ramp(g, k.positiveStyle).domain(0, c)
    };
    c = (new pv.Panel).data(function () {
        ...
        return d == "color" ? (k & 1) * f : 0
    }).fillStyle(function (k) {
        return k ? null : g
    });
    this.band =
        (new pv.Mark).top(function (k, l) {
            return d == "mirror" && l & 1 ? (l + 1 >> 1) * f : null
        }).bottom(function (k, l) {
            return d == "mirror" ? l & 1 ? null : (l + 1 >> 1) * -f : (l & 1 || -1) * (l + 1 >> 1) * f
        }).fillStyle(function (k, l) {
            return(l & 1 ? h : i)((l >> 1) + 1)
        });
    this.band.add = function (k) {
        return b.add(pv.Panel).extend(c).add(k).extend(this)
    }
};
...
pv.Layout.Rollup.prototype.buildImplied = function (b) {
    ...
    if (!pv.Layout.Network.prototype.buildImplied.call(this, b)) {
        var d = b.nodes, f = b.links, g = b.directed, h = d.length, i = [], j = [], k = 0, l = {}, q = {}, n = pv.Mark.stack, p = {parent: this};
        n.unshift(null);
        for (var m = 0; m < h; m++) {
            p.index = m;
            n[0] = d[m];
            i[m] = this.$x.apply(p, n);
            j[m] = this.$y.apply(p, n)
        }
        n.shift();
        ...
        for (m = 0; m <
            f.length; m++) {
            k = f[m].targetNode;
            d = l[c(f[m].sourceNode.index)];
            k = l[c(k.index)];
            h = !g && d.index > k.index ? k.index + "," + d.index : d.index + "," + k.index;
            (n = q[h]) || (n = q[h] = {sourceNode: d, targetNode: k, linkValue: 0, links: []});
            n.links.push(f[m]);
            n.linkValue += f[m].linkValue
        }
        b.$rollup = {nodes: pv.values(l), links: pv.values(q)}
    }
};
pv.Layout.Matrix = function () {
    ...
    this.label.data(function () {
        ...
        return this.index & 1 ? c * ((this.index >> 1) + 0.5) : 0
    }).top(function () {
        return this.index & 1 ? 0 : d * ((this.index >> 1) + 0.5)
...
pv.Layout.Matrix.prototype.buildImplied = function (b) {
    if (!pv.Layout.Network.prototype.buildImplied.call(this, b)) {
        var c = b.nodes, d = b.links, f = this.$sort, g = c.length, h = pv.range(g), i = [], j = [], k = {};
        b.$matrix = {labels: i, pairs: j};
        f && h.sort(function (m, r) {
            return f(c[m], c[r])
        });
        for (var l = 0; l < g; l++)for (var q = 0; q < g; q++) {
            var n = h[l], p = h[q];
            j.push(k[n + "." + p] = {row: l, col: q, sourceNode: c[n], targetNode: c[p], linkValue: 0})
        }
        for (l = 0; l < g; l++) {
            n = h[l];
            i.push(c[n], c[n])
...
pv.Layout.Bullet = function () {
    ...
    (this.marker = new pv.Mark).data(function () {
        ...
        return f == "left" ? d(k) : g ? null : this.parent.width() / 2
    }).top(function (k) {
        return f == "top" ? d(k) : g ? this.parent.height() / 2 : null
    }).right(function (k) {
        return f == "right" ? d(k) : null
    }).bottom(function (k) {
        return f == "bottom" ? d(k) : null
    ...
    (this.tick = new pv.Mark).data(function () {
        ...
        return f == "top" ? d(k) : null
    }).right(function (k) {
        return f == "right" ? d(k) : g ? null : -6
    }).bottom(function (k) {
        return f == "bottom" ? d(k) : g ? -8 : null
    }).height(function () {
        return g ? 6 : null
    }).width(function () {
        return g ? null : 6
...
pv.Behavior.drag = function () {
    function b(k) {
        ...
        f = this.scene;
        var l = this.mouse();
        i = ((h = k).fix = pv.vector(k.x, k.y)).minus(l);
        j = {x: this.parent.width() - (k.dx || 0), y: this.parent.height() - (k.dy || 0)};
    ...
    function c() {
        if (f) {
            f.mark.context(f, g, function () {
                var k = this.mouse();
                h.x = h.fix.x = Math.max(0, Math.min(i.x + k.x, j.x));
                h.y = h.fix.y = Math.max(0, Math.min(i.y + k.y, j.y));
                this.render()
            });
...
pv.Behavior.point = function (b) {
    function c(l, q) {
        l = l[q];
        q = {cost: Infinity};
        for (var n = 0, p = l.visible && l.children.length; n < p; n++) {
            var m = l.children[n], r = m.mark, s;
            if (r.type == "panel") {
                r.scene = m;
                ...
                for (x = m.length; t < x; t++) {
                    var u = m[t];
                    s = r.x - u.left - (u.width || 0) / 2;
                    u = r.y - u.top - (u.height || 0) / 2;
                    var o = i * s * s + j * u * u;
                    if (o < q.cost) {
                        q.distance = s * s + u * u;
                        q.cost = o;
    ...
    function d() {
        var l = c(this.scene, this.index);
        if (l.cost == Infinity || l.distance > k)l = null;
        if (g) {
            if (l && g.scene == l.scene && g.index == l.index)return;
            pv.Mark.dispatch("unpoint", g.scene, g.index)
        }
        if (g = l) {
            pv.Mark.dispatch("point", l.scene, l.index);
            pv.listen(this.root.canvas(), "mouseout", f)
        }
    }

    function f(l) {
        if (g && !pv.ancestor(this, l.relatedTarget)) {
            pv.Mark.dispatch("unpoint", g.scene, g.index);
            g = null
        }
    }

    var g, h = null, i = 1, j = 1, k = arguments.length ? b * b : 900;
    d.collapse = function (l) {
        if (arguments.length) {
            h = String(l);
            switch (h) {
                case "y":
    ...
    function d() {
        if (g) {
            g.mark.context(g, h, function () {
                var k = this.mouse();
                i.x = Math.max(0, Math.min(j.x, k.x));
                i.y = Math.max(0, Math.min(j.y, k.y));
                i.dx = Math.min(this.parent.width(), Math.max(k.x, j.x)) - i.x;
                i.dy = Math.min(this.parent.height(), Math.max(k.y, j.y)) - i.y;
                this.render()
            });
...
pv.Behavior.pan = function () {
    function b() {
        ...
        h = this.transform();
        j = 1 / (h.k * this.scale);
        if (k)k = {x: (1 - h.k) * this.width(), y: (1 - h.k) * this.height()}
    }

    function c() {
        if (f) {
            f.mark.context(f, g, function () {
                var l = h.translate((pv.event.pageX - i.x) * j, (pv.event.pageY - i.y) * j);
                if (k) {
                    l.x = Math.max(k.x, Math.min(0, l.x));
                    l.y = Math.max(k.y, Math.min(0, l.y))
                }
                this.transform(l).render()
            });
...
pv.Geo.scale = function (b) {
    ...
    var g = {x: 0, y: 0}, h = {x: 1, y: 1}, i = [], j = pv.Geo.projections.identity, k = pv.Scale.linear(-1, 1).range(0, 1), l = pv.Scale.linear(-1, 1).range(1, 0), q = {lng: 0, lat: 0}, n, p;
    c.x = function (m) {
        return c(m).x
    };
    c.y = function (m) {
    ...
    c.ticks = {lng: function (m) {
        ...
        if (i.length > 1) {
            ...
            if (m == undefined)m = 10;
            r = s.domain(i, function (t) {
                return t.lat
            }).ticks(m);
            m = s.domain(i, function (t) {
                return t.lng
            }).ticks(m)
        } else {
            r = pv.range(-80, 81, 10);
    ...
    c.invert = function (m) {
        return f({x: k.invert(m.x), y: l.invert(m.y)})
    };
    c.domain = function (m, r) {
        if (arguments.length) {
            ...
            if (i.length > 1) {
                ...
                q = {lng: (pv.max(s) + pv.min(s)) / 2, lat: (pv.max(t) + pv.min(t)) / 2};
                s = i.map(d);
