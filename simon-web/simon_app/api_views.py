...
def ntp_points(request):
    ...
    i = 0
    for point in points:
        point_dict = {'index': i, 'description': point.description, 'testtype': point.testtype, 'ip_address': point.ip_address, 'countryCode': point.country, 'date_added' : str(point.date_created)}
...
#             city = whoIs(point.ip_address)['entities'][0]['vcardArray'][1][2][3][3]
        ...
        for pointImage in pointImages:
            ...
            image_dict = {'path': pointImage.local_path, 'size': image.size, 'width': image.width, 'height': image.height, 'type': image.type, 'timeout': image.timeout, 'name' : image.name}
...
def country_latency_chart(request, country):
    ...
    results_country = Results.objects.filter((Q(country_origin=country) | Q(country_destination=country)) & Q(ave_rtt__gte=10))
    
    results_rtt = results_country.values_list('ave_rtt', flat=True).order_by('ave_rtt')#results_country.values_list('ave_rtt', flat=True).order_by('ave_rtt')
    ...
    for i in range(1, categories):
        ...
        for history_year in history_years:
            ...
            m = results_rtt.filter(Q(ave_rtt__gte=category_min) & Q(ave_rtt__lt=category_max) & Q(date_test__gte=date_from) & Q(date_test__lt=date_to)).count()
            try:
                row.append(float("%0.2f" % (100 * (m / float(n)))))
    ...
    for year in history_years:
        ...
        probes = "%s muestras" % (str(len(results_country.filter(Q(date_test__gt=date_from) & Q(date_test__lt=date_to)))))
...
def inner_latency_chart(request):
    ...
    for country in countries:
        results = Results.objects.filter(Q(country_origin=country) & Q(country_destination=country)).values_list('ave_rtt', flat=True)
        row = []
...
def region_throughput_chart(request):
    ...
    for categorized_bandwidth in categorized_bandwidths:
        ...
        if len(categorized_bandwidth) > 0:
            row.append(float("%0.2f" % ((categorized_bandwidth[len(categorized_bandwidth) - 1] + categorized_bandwidth[0]) / (2 * M))))  # Display in Mbps
        else:
            row.append(0)
...
def throughput_by_country_chart(request):
    ...
    table = []
    for country in countries:
        results = ThroughputResults.objects.filter(Q(country_origin=country) | Q(country_destination=country)).values('size', 'time')
        ...
        if len(results) is not 0:
            ...
            for result in results:
                if result['time'] is not 0:
                    throughput_results.append(result['size'] * 8000 / (result['time'] * M))  # list of throughput (bps), not {{size, time},{size, time},....}
...
def tables(request, country_iso, ip_version, year, month, tester, tester_version):  # , test_type):
    ...
    for country in target_countries:
        ...
        cursor.execute(settings.TABLES_QUERY , [country_iso, country, ip_version, datetime.datetime(year, month, day), tester, tester_version])  # , test_type])
        ...
        test_item = [country, number_probes, min_rtt, ave_rtt, max_rtt, dev_rtt, median_rtt, packet_loss, ip_version, tester, tester_version]
        keys = ['country_destination', 'number_probes', 'min_rtt', 'ave_rtt', 'max_rtt', 'dev_rtt', 'median_rtt', 'packet_loss', 'ip_version', 'tester', 'tester_version']
...
def throughput_json(request, country_iso, ip_version, year, month, tester, tester_version):  # , test_type):
    ...
    target_countries = ThroughputResults.objects.all().values_list('country_destination', flat='True').distinct().order_by()
    tests_list = []
...
#        cursor.execute(settings.THROUGHPUT_TABLES_QUERY , [country_iso, country, ip_version, datetime.datetime(year, month, day), tester, tester_version])
        cursor.execute(settings.THROUGHPUT_TABLES_QUERY , [country_iso, country, country, country_iso, ip_version, tester, tester_version])  # , test_type])
        country_test_subtable = cursor.fetchone()
        ...
        if country_test_subtable is not None:
            ...
            if country_test_subtable[3] is not None and country_test_subtable[4] is not None and country_test_subtable[4] is not 0:
        ...
        keys = ['ave_time', 'ave_size', 'country_destination', 'number_probes', 'ave_thput', 'std_dev', 'ip_version', 'tester', 'tester_version']
        tests_list.append(dict(zip(keys, test_item)))
