...
class Command(BaseCommand):
    def handle(self, *args, **options):
        ...
        try:
            ...
            tps = SpeedtestTestPoint.objects.distinct('country').order_by(
                'country')  # one TP per country TODO get *really* random tests...
            choice(tps)
            for tp in tps:
                ...
                ctx = Context(
                    {'cc': origins, 'count': count, 'destination': destination_ip, 'probeslimit': 10 * len(ccs),
                     'timeout': 30000})
                url = t.render(ctx)
